...
public class IntakeExtend extends CommandBase{
    ...
    public IntakeExtend(Arm a, double length) {
        generalConstructor(a, length, length);
    }

    public IntakeExtend(Arm a, double top, double bottom){
        generalConstructor(a, top, bottom);
    }

    private void generalConstructor(Arm arm, double top, double bottom) {
        this.arm = arm;
        topLength_target = top;
        bottomLength_target = bottom;
    ...
    public void initialize() {
        ...
        if(topLength_target > 20 && bottomLength_target > 20 && !arm.h.open){
            //topLength_target = 20;
   ...
   public static double calculate_next_step_length(double current_length, double suggested_length_step, double target_length) {
    ...
    {
        double current_length = arm.getTopWinchLength();
        double new_length=calculate_next_step_length(current_length,suggested_length_step_top,topLength_target);
        arm.topWinchSetLength(current_length+((new_length-current_length)/1.7));
    ...
    private void step_bottom(){
        double current_length = arm.getBottomWinchLength();
        double new_length=calculate_next_step_length(current_length,suggested_length_step_bottom,bottomLength_target);
        arm.bottomWinchSetLength(new_length);
    ...
    public void execute(){
        ...
        final double current_length_bottom=arm.getBottomWinchLength();
        final double current_length_top=arm.getTopWinchLength();
...
//        */
    ...
    public boolean isFinished() {
        ...
        */
       // /* 
        if((Math.abs(arm.getBottomWinchLength()-bottomLength_target)<1) && (arm.getTopWinchLength()-topLength_target)<1){
        return true;
        }
        else innerTimer.reset();
